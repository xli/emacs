; move-mode.el --- A major emacs mode for editing Move source code. Shamelessly copied from the rust version -*-lexical-binding: t-*-
...
(defconst move-mode-keywords
  '("abort" "acquires" "as" "assert" "copy" "borrow_global" "borrow_global_mut" "drop" "exists" "fun"
    "freeze" "friend" "has" "key" "module" "move_to" "move_from" "native" "public" "store" "script" "use"
    ;; Move spec lang keywords
    "aborts_if"
    "apply"
    "define"
    "except"
    ...
    "spec"
    "to"
    ;; end Move keywords, back into Move + Rust shared keywords
    "break"
    "const"
    "continue"
    "else"
...
(defconst rust-special-types
  '(
    "address" "signer" "u8" "u64" "u128" "vector"
    ;; end Move types, back into Rust types
    "bool"
    "char"))
...
(defvar move-mode-font-lock-keywords
  (append
   `(
     ;; Keywords proper
     (,(regexp-opt move-mode-keywords 'symbols) . font-lock-keyword-face)
  ...
  (let ((case-fold-search nil))
    (save-excursion
      ...
      (cond
       ...
       ((rust-looking-back-symbols move-mode-keywords)
...
(defun rust-syntax-propertize (start end)
  "A `syntax-propertize-function' for `move-mode'."
  (goto-char start)
...
\\{move-mode-map}"
  ...
  ;; Fonts
  (setq-local font-lock-defaults '(move-mode-font-lock-keywords
                                   nil nil nil nil
                                   (font-lock-syntactic-face-function . move-mode-syntactic-face-function)
                                   ))
...
(defun move-mode-reload ()
  (interactive)
  (unload-feature 'move-mode)
  (require 'move-mode)
  (move-mode))
...
(add-to-list 'auto-mode-alist '("\\.mvir\\'" . move-mode))
(add-to-list 'auto-mode-alist '("\\.move\\'" . move-mode))
